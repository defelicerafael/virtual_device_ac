...
5. **Swing/sleep con códigos separados (2026-07-13).** Si un código es toggle o separado depende de cada aire → se modela siempre como **códigos on/off separados**; si el aire en realidad tiene un solo código toggle, se repite el mismo código en ambas entradas de la planilla. En Homey quedan como toggles con estado.

## Preguntas abiertas

3. **Swing/sleep — comportamiento al apagar/prender.** Al apagar el AC, ¿swing/sleep se resetean o se conservan? ¿Y al prender: vale la misma regla que la temperatura (gana lo vigente en el tile)? Además: hoy `sleep` viaja DENTRO de la clave compuesta de la planilla (`{modo}_{fan}_{temp}_{sleep}`) — ¿sigue así, o pasa a ser comando separado on/off como swing (cambiaría el formato de la planilla a clave `{modo}_{fan}_{temp}` + filas `sleep_on`/`sleep_off`)?
