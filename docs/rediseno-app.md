...
7. **Learning por device (2026-07-13).** El learning deja de ser el botón global "Aprender AC": pasa a ser un **botón del propio device** que activa el modo learning de ese aire. Con el modo activo, el próximo comando va a `ac_learn` en lugar de `ac_command` (semántica del script actual: se apaga solo después de un comando — confirmar si queda así).
...
7. ~~Botón Learning~~ → RESUELTO (definición 7): botón por device. Queda: ¿cómo es el flujo `ac_learn` del lado servidor (aprende un comando por vez? guarda como `ir_{dev}_{clave}`?)? ¿Y el resultado aprendido termina en la planilla o solo en el servidor?
...
13. **"Modo/ON separado".** Propuesta nueva a partir del relevamiento: **autodetectar** mirando si el code tiene filas de modo solo (heat/cool sin temp), como ya pasa con el code 5 — sin flag manual en el wizard. Lo mismo aplicaría a "tiene fan_mode aprendido" (¿hay filas con fan ≠ auto?). ¿Va autodetección o flag manual? ¿El wait de 2s fijo está bien?
...
15. **Planilla / webservice.** ~~Formato y ejemplo~~ → RESUELTO (definición 12, [planilla-ir.md](planilla-ir.md)). Queda: ¿quién mantiene la planilla y cómo se cargan códigos nuevos (¿a mano desde capturas del Broadlink / desde lo aprendido con ac_learn?)? ¿La URL del Apps Script va hardcodeada o configurable a nivel app (settings de app)?
