...
| 21 | Funciones configurables por equipo (heat/dry/fan/sleep/velocidad) | 🔍 | `_syncAllowedFeatures` + `_assertModeAllowed`. **Falta verificación física** de que `setCapabilityOptions('thermostat_mode', {values})` realmente oculte los modos en el picker — quedó anotado como "a verificar en la prueba real" en la def. y nunca se registró el resultado. La red de seguridad del listener sí está |
...
- Verificación física del filtrado de modos del picker (def. 21).
- Imágenes definitivas de la app (hoy placeholders).
- Mejora menor anotada: cancelar reintentos supersedidos.
