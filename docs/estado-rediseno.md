...
**FALTA (lado servidor, Fernán):** que `script.ac_command` de cada casa devuelva `response_variable` `{ok, reason:'remote_unavailable'}` capturando si `remote.send_command` falló. Hasta entonces, con token la app no detecta remoteDown (respuesta vacía = ok). **Falta también probar en vivo end-to-end** una vez que el script devuelva la respuesta.
