...
**DESPLIEGUE (2026-07-15, confirmado por Fernán):** además del dev-run en San Fran (ferno), la app está **INSTALADA (persistente)** en los Homeys de clientes: **jx → "Sta Barbara OK"** (192.168.68.101) y **segun → "Cerrillos"** (192.168.80.16), ambos con la última versión (def. 24 incluida). Alta en `repos.txt` de cada server; actualizaciones futuras: `homey-app update com.panteasmart.devices` + `install` en cada server.
...
**Falta para el cierre (Etapa 7):** spot-checks pendientes (learning → ac_learn; 2 pasos con code 5; camino legacy sin code; swing con filas swing_* cuando se carguen en la planilla), imágenes definitivas, checklist contra las definiciones 1–24, mejora menor (cancelar reintentos supersedidos) y terminar la migración de los aires de San Fran.
