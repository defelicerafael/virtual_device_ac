...
## Última actualización: 2026-07-19 (madrugada: v2.0.3–2.0.5 + script enriquecido en ferno y base)

### 🔜 PENDIENTES AL RETOMAR (lo próximo, en orden de valor)
1. **Prueba de aceptación del feedback (def. 26) en San Fran — RE-PROBAR con v2.0.5.** Fernán, celular, "Prueba Zombie" (remote `test_claude` inexistente). **Cambiar el MODO** (a Cool), NO solo la temperatura (con el equipo en OFF y sin auto-on, la temperatura no envía y queda muda — def. 1). Esperado ahora: el tile **REVIERTE** al valor anterior + banner **"El control configurado no existe en Pantea Home Manager (remote.test_claude). Revisá la configuración del equipo."** (`remote_not_found` — el script de ferno ya lo distingue) + **notificación en el timeline (campanita)** + Telegram con la entidad. Botón "Reconectar control" en ⚙️ Ajustes del equipo → sección "Maintenance" (NO en el tile; migración v2.0.4 lo agrega a devices viejos), junto a "Recargar códigos". Botón "Aprender" ya NO debe verse en devices con code (v2.0.3). (Feedback ACTIVO solo en San Fran, token propio de ferno cargado.)
   - **v2.0.5 instalada en San Fran (ferno).** Los otros 3 Homeys siguen en versiones previas (jx 2.0.0+lock, segun/fatato pre-2.0.1) — actualizar cuando se confirme la prueba.
   - **Cambios v2.0.2→v2.0.5 (2026-07-18/19, feedback Fernán en vivo):** (a) v2.0.2 `remoteDown` REVIERTE + timeline; (b) v2.0.3 botón "Aprender" solo sin code (def. 7 rev.); (c) v2.0.4 migración `addCapability(button.reconnect)` para devices pre-def.26 (no aparecía en Ajustes→Maintenance); (d) v2.0.5 mensajes de error específicos por motivo (no_connection/service_missing/server_error/http_error + remote_not_found/remote_unavailable, con entidad; `_notifyFail` unifica banner+Telegram+timeline; 58 tests).
   - **Script `ac_command` ENRIQUECIDO y ACTIVO en ferno (2026-07-19):** distingue `remote_not_found` (entidad no registrada) de `remote_unavailable` (registrada pero unavailable/unknown). Backup `.bak-20260718-211717`, check_config OK, contenedor HA reiniciado (consultado) y verificado vivo sin errores. **También instalado en la IMAGEN BASE** (192.168.88.194, backup `.bak-20260718-211833`, HA apagado — lo toma el próximo clon). Referencia canónica: `docs/referencia/ha-script-ac_command.yaml`. Gotcha: los yaml de scripts en los servers son de root → sudo para backup/copiar.
