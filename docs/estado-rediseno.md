...
**DESPLIEGUE (actualizado 2026-07-16):** la app está **INSTALADA (persistente)** en los TRES Homeys: **San Fran** (ferno, 192.168.88.100 — pasó de dev-run a install el 16/7 porque cada redeploy reiniciaba los aires reales; `run` solo para debug puntual), **jx → "Sta Barbara OK"** (192.168.68.101) y **segun → "Cerrillos"** (192.168.80.16). Alta en `repos.txt` de cada server; actualizaciones: `homey-app update com.panteasmart.devices` + `install` en cada server. San Fran ya tiene migrados a la app nueva: Aire Escritorio, Aire Estar, Aire Cocina y Aire Cuarto.
