...
// Posiciones de swing (def. 5 v2): filas mode-only `swing_<posición>`.
const SWING_POSITIONS = ['auto', 'up', 'middle', 'down', 'off'];
const SWING_MODES = SWING_POSITIONS.map((p) => `swing_${p}`);
...
class IrCodes {
  ...
  /** Código de swing para una posición ('auto'|'up'|'middle'|'down'|'off'). */
  async getSwingCommand(code, position) {
    return this._getByKey(code, `swing_${position}`);
  }

  /** Todos los códigos de swing: { auto, up, middle, down, off } (null los que no estén). */
  async getSwingCommands(code) {
    const result = {};
    for (const position of SWING_POSITIONS) {
      result[position] = await this._getByKey(code, `swing_${position}`);
    }
    return result;
  }
