...
const SWING_MODES = ['swing_on', 'swing_off'];
...
class IrCodes {
  ...
  /** Códigos de swing: { on, off } (null los que no estén). */
  async getSwingCommands(code) {
    return {
      on: await this._getByKey(code, 'swing_on'),
      off: await this._getByKey(code, 'swing_off'),
    };
  }
