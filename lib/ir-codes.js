...
// Auto-apagado (def. 28): también filas mode-only, con el tiempo en DÉCIMAS
// de hora y al menos dos dígitos — `timer_off_05` = 0,5 h, `timer_off_10` =
// 1 h, `timer_off_15` = 1,5 h, `timer_off_120` = 12 h. Sin decimales en la
// columna `mode` de la planilla (Sheets no las convierte a número) y sin
// ambigüedad de coma vs. punto.
const TIMER_PREFIX = 'timer_off_';
...
class IrCodes {
  ...
  /** Clave de planilla del auto-apagado de X horas (def. 28). 0 = cancelar. */
  static timerKey(hours) {
    const tenths = Math.round(Number(hours) * 10);
    return `${TIMER_PREFIX}${String(tenths).padStart(2, '0')}`;
  }

  /** Horas de una clave `timer_off_XX`; null si la clave no es de auto-apagado. */
  static timerHours(key) {
    if (typeof key !== 'string' || !key.startsWith(TIMER_PREFIX)) return null;
    const digits = key.slice(TIMER_PREFIX.length);
    if (!/^\d+$/.test(digits)) return null;
    return Number(digits) / 10;
  }

  /** Código de auto-apagado para X horas (0 = comando de cancelar). */
  async getTimerCommand(code, hours) {
    return this._getByKey(code, IrCodes.timerKey(hours));
  }

  /**
   * Tiempos de auto-apagado aprendidos para un code, en horas y ordenados
   * (sin el 0, que es el comando de cancelar). Con esto el device decide el
   * máximo y el paso del slider: si son todos enteros, no ofrece medias horas.
   */
  async getTimerHours(code) {
    const entry = await this._load(code);
    if (!entry) return [];
    return Object.keys(entry.commands)
      .map((key) => IrCodes.timerHours(key))
  ...
  async getSummary(code) {
    ...
    const modes = uniq(entry.rows.map((r) => String(r.mode))
      .filter((m) => m && !SWING_MODES.includes(m) && IrCodes.timerHours(m) == null));
    ...
    return {
      ...
      timerHours: await this.getTimerHours(code),
...
module.exports.TIMER_PREFIX = TIMER_PREFIX;
