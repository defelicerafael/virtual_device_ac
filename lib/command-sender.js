...
class CommandSender {
  ...
  /**
   * Comando de swing (def. 5 v2): posición 'auto'|'up'|'middle'|'down'|'off'.
   * Solo si la planilla tiene el código de esa posición; sin código no se
   * envía nada ({ok: true, skipped: true}).
   */
  async sendSwing(config, position) {
    const command = await this._ir.getSwingCommand(config.code, position);
    if (!command) {
      this._log(`swing_${position}: sin código en la planilla, no se envía`);
  ...
  /**
   * Learning de swing (def. 5 v2 + 7): manda a ac_learn la posición a
   * aprender. ⚠️ Contrato nuevo con el servidor (el script de ac_learn debe
   * soportar `swing`).
   */
  async sendSwingLearn(config, position) {
